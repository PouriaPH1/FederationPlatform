...
namespace FederationPlatform.IntegrationTests.Flows
{
    public class ActivityManagementFlowTests : IntegrationTestBase
    {
        ...
        public async Task PendingActivitiesNotification_AdminReceivesNotification()
        {
            ...
            // Assert - Admin should have received notification
            var adminNotifications = await _notificationService.GetUserNotificationsAsync(admin.Id);
            adminNotifications.Should().NotBeEmpty();
            adminNotifications.Should().Contain(x => x.Message.Contains(\"new activity\"));
        }

        [Fact]
        public async Task ActivityApprovalNotification_RepresentativeReceivesNotification()
        {
            // Arrange
            ...
            var createDto = new CreateActivityDto
            {
                Title = \"Activity for Approval Notification\",
            ...
            // Assert - Representative should receive notification
            var notifications = await _notificationService.GetUserNotificationsAsync(representative.Id);
            notifications.Should().Contain(x => x.Message.Contains(\"approved\"));
        }
    }
}
